...
menjadi make_l4q_conv2d / make_l4q_linear, kecuali di dalam definisi helper itu sendiri.
"""
...
inside_make_l4q_linear = inside_make_l4q_conv2d = False
...
for line in src:
    ...
    if stripped.startswith("def make_l4q_linear"):
        inside_make_l4q_linear = True
    elif stripped.startswith("def make_l4q_conv2d"):
        inside_make_l4q_conv2d = True
    elif stripped.startswith("def "):  # fungsi lain => helper selesai
        inside_make_l4q_linear = inside_make_l4q_conv2d = False

    if not inside_make_l4q_linear:
        line = pat_linear.sub(r'\1make_l4q_linear(', line)

    if not inside_make_l4q_conv2d:
        line = pat_conv.sub(r'\1make_l4q_conv2d(', line)
